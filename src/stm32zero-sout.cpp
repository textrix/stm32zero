 ...
 *   - STM32ZERO_STDOUT_UART: UART handle name (required)
 *   - STM32ZERO_STDOUT_BUFFER_SIZE: Buffer size (default: 4096)
...
#ifndef STM32ZERO_STDOUT_UART
#error "STM32ZERO_STDOUT_UART must be defined in stm32zero-conf.h"
#endif

#ifndef STM32ZERO_STDOUT_BUFFER_SIZE
#define STM32ZERO_STDOUT_BUFFER_SIZE	4096
#endif

#ifndef STM32ZERO_STDOUT_CACHE_CLEAN
#define STM32ZERO_STDOUT_CACHE_CLEAN	0
#endif

// UART handle declaration (generated by STM32CubeMX)
extern UART_HandleTypeDef STM32ZERO_STDOUT_UART;
...
#if STM32ZERO_STDOUT_CACHE_CLEAN
		if constexpr (cache_line_size > 0) {
			SCB_CleanDCache_by_Addr(
				reinterpret_cast<uint32_t*>(const_cast<uint8_t*>(buffers_[tx_idx])),
				aligned_size());
		}
#endif

		HAL_UART_Transmit_DMA(&STM32ZERO_STDOUT_UART, const_cast<uint8_t*>(buffers_[tx_idx]), tx_len);
...
STM32ZERO_DMA_TX static DmaBuffer<STM32ZERO_STDOUT_BUFFER_SIZE> tx_buf0_;
STM32ZERO_DMA_TX static DmaBuffer<STM32ZERO_STDOUT_BUFFER_SIZE> tx_buf1_;

STM32ZERO_DTCM static DualBuffer<STM32ZERO_STDOUT_BUFFER_SIZE> dual_buffer_;
...
void init()
{
	HAL_UART_RegisterCallback(&STM32ZERO_STDOUT_UART, HAL_UART_TX_COMPLETE_CB_ID, uart_tx_complete_callback);
}
...
{
	(void)file;
	return stm32zero::sout::write(ptr, len);
}
